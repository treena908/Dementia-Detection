...
def generate_full_interview_mmse():
    ...
    for label in ["Control", "Dementia"]:
        ...
        PATH = "data/Pitt_transcripts/" + label + "/cookie"
        for path, dirs, files in os.walk(PATH):
        ...
        for folder in folders:
            ...
            for path, dirs, files in os.walk(PATH):
                for filename in files:
                    fullpath = os.path.join(path, filename)
                    with open(fullpath, 'r')as input_file:
                        id,age,sex,tokenized_list = file_tokenization_utterances(input_file)
                        for element in tokenized_list:
                            dementia_list.append(
                                {'text': element,
                                 'label': label,
...
#dataframe = generate_full_interview_mmse()
#dataframe = process_mmse_full_interview()
dict={'text1':[],'text2':[],'mmse1':[],'mmse2':[],'feature1':[],'feature2':[],'id1':[],'id2':[]}
df = pd.read_pickle('data/pitt_full_interview_features.pickle')
# writer = pd.ExcelWriter("data/pitt_mmse_feature.xlsx", engine='xlsxwriter')
# # writer = pd.ExcelWriter("karnekal_niu_bansal_2018/results/mmse_smote_result.xlsx", engine='xlsxwriter')
# #
# df.to_excel(writer, sheet_name='Sheet1', columns=['text1','text2','feature1','feature2','mmse1','mmse2','id1','id2'])
# writer.save()
#print(df.iloc[0,:])
...
with open('data/pitt_mmse_features_new.pickle', 'wb') as f:
      pickle.dump(df_new_2, f)
